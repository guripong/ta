        ...
        if (QuerryLoad_Possible == 1) {
            ...
            mysql.createConnection(config).then(function (conn) {
                ...
                if (results == -1) {
                    ...
                    {
                        if (parameters.set_start == 1) {//같은 셋트 qn을 긁어온상태
                ...
                if (results == -1) {
                    ...
                    {//셋트의경우임  순서에따른 새로운 question을 긁어온상태임 
                        if (parameters.set_start == 2) { //question 처음이라고 보시면됨
                ...
                else if (parameters.is_set == 'N') //세트로 안묵는경우
                {
                    ...
                    else //G C E M S
                    {
                        ...
                        set_total_speech(total_speech, parameters.Speed_S, parameters).then(function (results) {
                            ...
                            conv.ask('<speak>' + parameters.Speed_S + parameters.total_speech +'<desc>Sound Error</desc>'+ Speed_E + '</speak>');
...
ap.intent('Answer', (conv, input) => {
    ...
    return new Promise(function (resolve_answer) {
        ...
        mysql.createConnection(config).then(function (conn) {
            ...
            else if (type == 'G' || type == 'D' || type == 'R') // choose_case_num확인필요
            {
                ...
                var tmpmyspeak = speak.split(` `);
                if (Array.isArray(case_array)) {//배열일경우에만
                    for (var i = 0; i < case_array.length; i++) {
                        isjungdab = 0;
                        // tmpmyspeak_index=0;
                        for (var j = 0; j < case_array[i].length; j++) {
                            ...
                            for (var k = 0; k < tmpjungdab.length; k++) {
                                ...
                                {
                                    if (k + 1 == tmpjungdab.length && tmpjungdab[k] == tmpmyspeak[tmpmyspeak_index] && speak.length == case_array[i][j].length) {//tmpjungdab은 정답case를 단어별로 쪼갠거에요   tmpmyspeak는 내가 말한걸가따가 쪼갠거고
            ...
            if (results == -1) {
                ...
                resolve_answer('error can not load type!@!@');
            }
            else {
